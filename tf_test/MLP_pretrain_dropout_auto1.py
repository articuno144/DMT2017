...
desired_dataframe = pd.read_csv('desired_pretrain.csv',sep = ',', header = None)
desired_values = desired_dataframe.values[:,0]
desired_values = np.transpose(desired_values.reshape((5,-1)))

input_dataframe = pd.read_csv('input_pretrain.csv',sep = ',',header = None)
...
with tf.Session() as sess:
    ...
    for i in range(700):
        t_i = i+305;
        cost[i] = sess.run(pred, feed_dict = {x: evaluate_cost(int2,t_i), keep_prob:1.0})
